...
class LostCitiesGui:
    ...
    def start(self, first, second, *, seed=0, minimum_move_seconds=.4, save_trace=False):
        if any(p.kind not in ('human', 'random') for p in (first, second)):
        ...
        with self._condition:
            ...
            self._state = {**position.to_dict(), 'status': 'playing', 'session_id': self._session_id,
                           ...
                           'players': [{'kind': p.kind} for p in (first, second)], 'winner': None}
            self._thread = threading.Thread(target=self._run,
                args=(game, position, (first, second), seed, minimum_move_seconds, cancelled), daemon=True)
            self._thread.start()
    ...
    def _run(self, game, position, players, seed, delay, cancelled):
        ...
        try:
            while not cancelled.is_set():
                ...
                with self._condition:
                    if cancelled.is_set():
                        return
                    self._state['events'].append({'player': active, 'chance': chance, 'action': action.to_dict()})
                    self._state['turn'] += 1
                    self._state['status'] = 'playing'
                    self._state['legal_actions'] = []
