...
class SplendorGui:
    ...
    def _decorate(self, state, status):
        return {**state, "session_id": self._session_id, "status": status, "players": [p.as_dict() for p in self._players],
    ...
    def start(self, first, second, *, seed=0, minimum_move_seconds=0.4, save_trace=False):
        ...
        session = SplendorSession(seed=seed, first=self._agent(first), second=self._agent(second))
        with self._condition:
            self.cancel()
    ...
    def submit_move(self, action, turn, session_id):
        ...
        with self._condition:
            if session_id != self._session_id or self._state["status"] != "waiting_human" or turn != len(self._state["events"]):
    ...
    def _run(self, session, cancelled, delay, save_trace, seed, players):
        try:
            while not cancelled.is_set():
                ...
                status = "finished" if state["finished"] else "waiting_human" if state["waiting_human"] else "playing"
                with self._condition:
                    if cancelled.is_set():
                        return
            ...
            if save_trace and not cancelled.is_set():
                ...
                path = self._trace_dir / f"splendor-{seed}-{uuid4().hex}.json"
                payload = {"format": "splendor_session_v1", "seed": seed, **session.snapshot(),
