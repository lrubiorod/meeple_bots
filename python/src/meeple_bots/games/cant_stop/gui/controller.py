"""Cancellable GUI worker for explicitly interleaved decisions and chance events."""
...
class CantStopGui:
    def __init__(self, trace_dir=Path("results/gui/cant-stop")):
        ...
        self._players = (GuiPlayer("human"), CANT_STOP_BASELINE)
        self._state = self._decorate(CantStopSession().snapshot(), "idle")
        self._pending = None

    def _decorate(self, state, status):
        return {**state, "status": status, "players": [p.as_dict() for p in self._players],
    ...
    def start(self, first, second, *, seed=0, minimum_move_seconds=0.4, save_trace=False):
        ...
        session = CantStopSession(seed=seed, first=self._agent(first), second=self._agent(second))
        with self._condition:
            self.cancel()
    ...
    def submit_move(self, action, turn):
        ...
        with self._condition:
            if self._state["status"] != "waiting_human" or turn != len(self._state["events"]):
    ...
    def _run(self, session, cancelled, delay, save_trace, seed, players):
        try:
            while not cancelled.is_set():
                ...
                status = "finished" if state["winner"] is not None else "waiting_human" if state["waiting_human"] else "playing"
                with self._condition:
                    if cancelled.is_set():
                        return
            ...
            if save_trace and not cancelled.is_set():
                ...
                path = self._trace_dir / f"cant-stop-{seed}-{uuid4().hex}.json"
                payload = {"format": "cant_stop_session_v1", "seed": seed, **session.snapshot(),
