...
<body>
      ...
      <section class="panel config">
        ...
        <div class="players">
          <div class="player" id="player-card-0" style="--player-color: var(--x)">
            ...
            <div class="mcts-options hidden" id="mcts-0"><label>Selección<select id="selection-policy-0"><option value="uct">UCT</option><option value="ucb1_tuned">UCB1-Tuned (ignora exploración)</option></select></label><label>Presupuesto<select id="budget-mode-0"><option value="iterations">Iteraciones</option><option value="time">Tiempo</option></select></label><label class="budget-field" id="iterations-label-0">Iteraciones<input id="iterations-0" type="number" min="1" value="1000"></label><label class="budget-field hidden" id="time-label-0">Tiempo por decisión (s)<input id="time-budget-0" type="number" min="0.001" step="0.1" value="1"></label><label>Profundidad<input id="depth-0" type="number" min="1" value="9"></label><label><input id="tree-reuse-0" type="checkbox"> Reutilizar árbol</label></div>
          </div>
          <div class="player" id="player-card-1" style="--player-color: var(--o)">
            ...
            <div class="mcts-options" id="mcts-1"><label>Selección<select id="selection-policy-1"><option value="uct">UCT</option><option value="ucb1_tuned">UCB1-Tuned (ignora exploración)</option></select></label><label>Presupuesto<select id="budget-mode-1"><option value="iterations">Iteraciones</option><option value="time">Tiempo</option></select></label><label class="budget-field" id="iterations-label-1">Iteraciones<input id="iterations-1" type="number" min="1" value="1000"></label><label class="budget-field hidden" id="time-label-1">Tiempo por decisión (s)<input id="time-budget-1" type="number" min="0.001" step="0.1" value="1"></label><label>Profundidad<input id="depth-1" type="number" min="1" value="9"></label><label><input id="tree-reuse-1" type="checkbox"> Reutilizar árbol</label></div>
          </div>
        </div>
  ...
  <script>
    ...
    for (const player of [0, 1]) {
      ...
      document.querySelector(`#budget-mode-${player}`).addEventListener('change', () => updateBudgetFields(player));
    }
    ...
    function playerConfig(index) {
      ...
      return {
        kind,
        selection_policy: document.querySelector(`#selection-policy-${index}`).value,
        iterations: timed ? null : Number(document.querySelector(`#iterations-${index}`).value),
    ...
    async function start() {
      ...
      try {
        state = await api('/api/start', {
          ...
          body: JSON.stringify({first: playerConfig(0), second: playerConfig(1), seed: Number(document.querySelector('#seed').value), minimum_move_seconds: Number(pace.value), save_trace: document.querySelector('#save-trace').checked}),
        });
        render();
